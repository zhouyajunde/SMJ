...
const int YILTableViewHeaderH = 10;

@interface ZYJMineViewController ()

@end

@implementation ZYJMineViewController
...
//    tableView.backgroundColor =  [UIColor colorWithPatternImage:[UIImage imageNamed:@"tuijian_bg"]];
    ...
    tableView.sectionHeaderHeight = YILTableViewHeaderH;
    
    if (iOS7) {
        tableView.contentInset = UIEdgeInsetsMake(YILTableViewHeaderH-35, 0, 0, 0);
    }
...
//        }
        ...
        if ((long)indexPath.row == 7) {
            [self performSegueWithIdentifier:@"own2Change" sender:nil];
        }else if ((long)indexPath.row == 8){
            [self performSegueWithIdentifier:@"own2duihuan" sender:nil];
        }
    }
}
